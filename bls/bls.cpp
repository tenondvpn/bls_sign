...
libff::alt_bn128_G1 Bls::Signing(
    ...
    return secret_key.as_bigint() * hash;  // sign
                                                                     /*
...
bool Bls::Verification( const libff::alt_bn128_G1& hash, const libff::alt_bn128_G1& sign,
    const libff::alt_bn128_G2& public_key, libff::alt_bn128_GT* res ) {
        ...
        *res =
        ...
        auto res1 = libff::alt_bn128_ate_reduced_pairing( hash, public_key );
        if ( *res == res1 ) {
            return true;
        }
    ...
    bool Bls::Verification( const libff::alt_bn128_G1& hash, const libff::alt_bn128_G1& sign,
        ...
        // there are several types of pairing, it does not matter which one is chosen for
        // verification
    }

    bool Bls::GetVerifyHash( const libff::alt_bn128_G1& hash, const libff::alt_bn128_G2& public_key,
        ...
        *res = libff::alt_bn128_ate_reduced_pairing( hash, public_key );
        return true;
    }

    bool Bls::GetVerifyHash(const libff::alt_bn128_G1& sign,
        libff::alt_bn128_GT* res) {
        libff::inhibit_profiling_info = true;
        ...
        *res = libff::alt_bn128_ate_reduced_pairing( sign, libff::alt_bn128_G2::one() );
        return true;
    }
    ...
    bool Bls::AggregatedVerification(
        ...
        return (
            libff::alt_bn128_ate_reduced_pairing( aggregated_sig, libff::alt_bn128_G2::one() ) ==
    ...
    std::pair< libff::alt_bn128_Fr, libff::alt_bn128_G2 > Bls::KeysRecover(
        ...
        if ( shares.size() < this->t_ || coeffs.size() < this->t_ ) {
            throw ThresholdUtils::IncorrectInput(
                "not enough participants in the threshold group" );
        }
        ...
        for ( size_t i = 0; i < this->t_; ++i ) {
            ...
            secret_key +=
                coeffs[i] * shares[i];  // secret key recovering using Lagrange Interpolation
        }
    ...
    libff::alt_bn128_G1 Bls::SignatureRecover( const std::vector< libff::alt_bn128_G1 >& shares,
        ...
        for ( size_t i = 0; i < this->t_; ++i ) {
            if ( !shares[i].is_well_formed() ) {
                throw ThresholdUtils::IsNotWellFormed(
                    "incorrect input data to recover signature" );
            }
            sign =
                sign + coeffs[i] * shares[i];  // signature recovering using Lagrange Coefficients
        }
