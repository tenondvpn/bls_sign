...
then
	...
	then
		...
		if [ "$UNIX_SYSTEM_NAME" = "Darwin" ];
		then
			eval ./configure CC="/usr/bin/gcc -std=c11" "${CONF_CROSSCOMPILING_OPTS_GENERIC}" "${CONF_DEBUG_OPTIONS}" --enable-cxx --enable-static --disable-shared --build=x86_64-apple-darwin#{OS.kernel_version.major} --prefix="$INSTALL_ROOT"
		else
			if [[ "${WITH_EMSCRIPTEN}" -eq 1 ]];
			then
				CC="/usr/bin/gcc -std=c11" eval emconfigure ./configure "${CONF_CROSSCOMPILING_OPTS_GENERIC}" "${CONF_CROSSCOMPILING_OPTS_GMP}" "${CONF_DEBUG_OPTIONS}" --disable-assembly --host none --enable-cxx --prefix="$INSTALL_ROOT"
			else
				CC="/usr/bin/gcc -std=c11" eval ./configure "${CONF_CROSSCOMPILING_OPTS_GENERIC}" "${CONF_CROSSCOMPILING_OPTS_GMP}" "${CONF_DEBUG_OPTIONS}" --enable-cxx --enable-static --disable-shared --prefix="$INSTALL_ROOT"
			fi
		fi
		echo -e "${COLOR_INFO}building it${COLOR_DOTS}...${COLOR_RESET}"
		if [[ "${WITH_EMSCRIPTEN}" -eq 1 ]];
		...
		then
			...
			then
				...
				cd argtable2
                        ...
                        sed -i 's/static bool isspace/static int isspace/g' ./src/arg_int.c 
			echo -e "${COLOR_INFO}building it${COLOR_DOTS}...${COLOR_RESET}"
			cd build
			$MAKE ${PARALLEL_MAKE_OPTIONS}
			$MAKE ${PARALLEL_MAKE_OPTIONS} install
...
then
        ...
        then
				...
				sed -i 's/HttpServer\:\:callback\,/(MHD_AccessHandlerCallback)HttpServer\:\:callback\,/g' ../src/jsonrpccpp/server/connectors/httpserver.cpp
